...
var BMSite = new(function() {
    ...
    this.siteYAddSpec = 0;
    this.siteScale = 1;
    this.sitesArray = ["INTRO", "ATTITUDE", "CURRICULUM", "FEE STRUCT", "youandwe", "contact", "projects"];
    this.sitesTitlesPL = ["", "Nastawienie", "Kreacja", "FEE STRUCT", "Ty i my", "Kontakt", "To zrobili\u015bmy"];
    this.sitesTitlesEN = ["", "ATTITUDE", "CURRICULUM", "FEE STRUCT", "You and we", "Contact", "Our work"];
...
$.ready(function() {
    ...
    naviNumbers = document.getElementById("naviNumbers");
...
function handleHashChange() {
    ...
    if (!isTouch) TweenLite.to(window, 0, { scrollTo: { y: BMSite.currentScrollY }, overwrite: "all", onUpdate: scrollHistoryUse });
    ...
    if (!isTouch) {
        if (dist == 1) tl.to(window, dist * 1.5, { scrollTo: { y: BMSite.currentScrollY + pos * 200 * BMSite.siteScale }, ease: SlowMo.ease, onUpdate: scrollHistoryUse });
        tl.to(window, dist * 0.5, { scrollTo: { y: BMSite.currentScrollY }, ease: Power4.easeOut, onUpdate: scrollHistoryUse });
    } else {
        Scroller.targetYUpdateTmp();
        if (dist == 1) tl.to(Scroller, dist * 1.5, { targetYTmp: (newPosition + pos * 200) * BMSite.siteScale, ease: SlowMo.ease, onUpdate: updateWithoutHistory });
        tl.to(Scroller, dist * 0.5, { targetYTmp: newPosition * BMSite.siteScale, ease: Power4.easeOut, onUpdate: updateWithoutHistory });
    }
}
...
function handleHashChangeInit() {
    ...
    if (isNaN(BMProject.selectedProject)) BMProject.selectedProject = 0;
    if (BMProject.selectedProject >= BMProject.projects.length) BMProject.selectedProject = 0;
    if (stateStr != "projects") History.replaceState({ state: State.data.state }, titleT + "Ethical learning", "?lang=" + BMSite.lang.toLowerCase() + "&site=" + stateStr);
    else History.replaceState({ state: State.data.state, id: BMProject.selectedProject }, titleT + "Ethical learning", "?lang=" + BMSite.lang.toLowerCase() + "&site=" + stateStr + "&id=" + BMProject.selectedProject);
    if (idSite == 6) {
        newPosition = 0;
        BMSite.currentScrollY = newPosition * BMSite.siteScale;
        BMSite.currentSite = 0;
    ...
    switch (idSite) {
        case 1:
            if (siteVersion != 1) newPosition = 1600;
    ...
    if (isTouch) {
        ...
        mouseOut1(null);
        Scroller.targetYUpdateTmp();
        TweenLite.to(Scroller, 0, { targetYTmp: BMSite.currentScrollY, overwrite: "all", ease: Power4.easeOut, onUpdate: updateTargetScrollContact });
    }
}
...
function setHashSilentlyProject(hash) {
    ...
    History.pushState({ state: hash, id: BMProject.selectedProject }, titleT + "Ethical learning", "?lang=" + BMSite.lang.toLowerCase() + "&site=" + hash + "&id=" + BMProject.selectedProject);
}
...
function setHashSilently(hash) {
    ...
    BMSite.triggerHistory = false;
    BMSite.oldSite = History.getState().data.state;
    History.pushState({ state: hash }, titleT + "Ethical learning", "?lang=" + BMSite.lang.toLowerCase() + "&site=" + hash);
}
    ...
    if (navigator.platform.indexOf("iPad") != -1) {
        ...
        if (event.acceleration) {
            ...
            if (DetectIt.landscapeOrientation) {
                ...
                tempY = parseInt(event.accelerationIncludingGravity.y * 150);
            }
        ...
        if (needsRedrawProject)
            if (siteVersion == 0) {
                ...
                if (BMSite.supportsTransforms) {
                    setElementTransformM(projectText, parseInt((browser_width - 380) / 2 - (BMSite.tempXT - parseInt(browser_width / 2)) / 20) + "px", parseInt(browser_height / 2 + 170 * BMProject.siteScale) + "px");
                    ...
                    set2dTransformM(containerPages, parseInt((browser_width - 800 * BMProject.siteScale) / 2 - (BMSite.tempXT - parseInt(browser_width / 2)) / 15) + "px", parseInt(browser_height / 2 - 20 * BMProject.siteScale) + "px");
...
function setContentPosition() {
    ...
    if (canvasMainBgObj.gfx2) {
        ...
        if (BMSite.currentScrollYTmp.toFixed(1) == BMSite.currentScrollYS.toFixed(1)) {
            needsDoScrollJob = false;
            return;
        }
    }
}
...
function parallaxScroll(e) {
    ...
    if (mouseWheelActive) {
        ...
        if (delta < 0) TweenLite.to(window, 1.5, { scrollTo: { y: BMSite.currentScrollY + 250 }, overwrite: "all", ease: Power4.easeOut, onUpdate: scrollSpecialUse });
        else if (BMSite.currentScrollY - 250 > 0) TweenLite.to(window, 1.5, { scrollTo: { y: BMSite.currentScrollY - 250 }, overwrite: "all", ease: Power4.easeOut, onUpdate: scrollSpecialUse });
        else {
            BMSite.currentScrollY = 0;
            TweenLite.to(window, 1.5, { scrollTo: { y: BMSite.currentScrollY }, overwrite: "all", ease: Power4.easeOut, onUpdate: scrollSpecialUse });
        }
...
var deviceDetection = (function() {
    ...
    return { isSmartphoneOrTablet: isSmartphoneOrTablet, device: device, osVersion: osVersion, userAgent: userAgent, deviceType: deviceType };
})();
...
function updateValues() {
    ...
    site_height = 8100 * BMSite.siteScale + BMSite.siteYAdd * 2;
    ...
    contentMain.style.left = parseInt((browser_width - BMSite.siteWidthC) / 2) + "px";
    ...
    projectOpen1.style.top = 2038 * BMSite.siteScale + "px";
    ...
    projectOpen3.style.top = 5120 * BMSite.siteScale + "px";
    ...
    if (roseContainer != null) {
        ...
        techContainer.style.width = BMSite.siteScale * 549 * 1.6 + "px";
        ...
        crownContainer.style.left = parseInt((BMSite.siteWidthC - crownContainer.offsetWidth) / 2) + "px";
    ...
    n5roll.style.height = parseInt(42 * BMSite.siteScale) + "px";
    ...
    if (parseInt(17 * BMSite.siteScale) > 11) {
        ...
        n2r.style.fontSize = parseInt(17 * BMSite.siteScale) + "px";
    ...
    if (siteVersion != 1) {
        ...
        CURRICULUM.style.left = parseInt(introW.offsetWidth + dist) + "px";
        quality.style.left = parseInt(creationW.offsetWidth + dist + CURRICULUM.offsetLeft) + "px";
        ...
        CURRICULUM.style.left = parseInt((BMSite.siteWidthN - creationW.offsetWidth - 28) / 2) + "px";
        ...
        CURRICULUM.style.top = distHM * 2 + "px";
    ...
    if (parseInt(55 * BMSite.siteScale) > 15) {
        ...
        technologyHeader.style.fontSize = parseInt(55 * BMSite.siteScale) + "px";
    ...
    creationHeader.style.letterSpacing = parseInt(18 * BMSite.siteScale) + "px";
    ...
    if (siteVersion == 0) {
        ...
        if (siteVersion == 1) {
            ...
            qualityHeader.style.left = parseInt((BMSite.siteWidthC - qualityHeader.offsetWidth) / 2) + "px";
            ...
            solutionsHeader.style.top = parseInt((4130 + 350 + 900 + 650) * BMSite.siteScale) + "px";
    ...
    if (parseInt(14 * BMSite.siteScale) > 12) {
        ...
        textContainer4.style.fontSize = parseInt(14 * BMSite.siteScale) + "px";
        ...
        textContainer2.style.fontSize = "12px";
    ...
    if (siteVersion == 1) {
        ...
        textContainer5.style.textAlign = "center";
        ...
        for (var z1 = 1; z1 < 9; z1++) {
            var t2 = document.getElementById("text2_" + z1);
        ...
        textContainer4.style.width = BMSite.siteWidthC + "px";
        ...
        textContainer5.style.left = parseInt((BMSite.siteWidthC - textContainer5.offsetWidth) / 2) + "px";
    } else {
        ...
        textContainer3.style.top = parseInt(3586 * BMSite.siteScale) + "px";
        ...
        for (var z4 = 1; z4 < 9; z4++) document.getElementById("text5_" + z4).style.textIndent = (z4 - 1) * 21 + "px";
    }
    ...
    if (BMSite.siteScale < 1) {
        ...
        soundCont.style.left = parseInt(720 * BMSite.siteScale) + "px";
    }
    if (siteVersion == 1) {
